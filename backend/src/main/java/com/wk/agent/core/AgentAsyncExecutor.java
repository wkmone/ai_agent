...
import com.wk.agent.service.impl.AgentServiceImpl;
...
@Service
public class AgentAsyncExecutor {

    private static final Logger log = LoggerFactory.getLogger(AgentAsyncExecutor.class);

    @Value("${agent.async.thread-pool-size:10}")
    private int threadPoolSize;

    @Value("${agent.async.timeout-seconds:300}")
    ...
    @Autowired
    private AgentSharedStateService sharedStateService;
    ...
    public void initialize() {
        this.executorService = Executors.newFixedThreadPool(threadPoolSize, r -> {
            ...
            t.setName("agent-async-" + t.getId());
            t.setDaemon(true);
            return t;
        });
        log.info("Agent 异步执行器初始化完成，线程池大小: {}", threadPoolSize);
    }
    ...
    public CompletableFuture<AgentResult> executeAsync(String agentId, AgentTask task, int timeoutSeconds) {
        ...
        }, executorService).orTimeout(timeoutSeconds, TimeUnit.SECONDS).exceptionally(ex -> {
    ...
    public Map<String, Object> getStats() {
        ...
        for (AsyncExecutionContext context : executionContexts.values()) {
            statusCounts.merge(context.getStatus().name(), 1L, Long::sum);
        }
    ...
    public void cleanupCompletedExecutions() {
        executionContexts.entrySet().removeIf(entry -> {
            AsyncExecutionContext context = entry.getValue();
            return context.getStatus() == AsyncExecutionStatus.COMPLETED ||
                   context.getStatus() == AsyncExecutionStatus.FAILED ||
                   context.getStatus() == AsyncExecutionStatus.CANCELLED ||
                   context.getStatus() == AsyncExecutionStatus.TIMEOUT;
        });
    }
