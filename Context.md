# [第九章 上下文工程](https://datawhalechina.github.io/hello-agents/#/./chapter9/第九章 上下文工程?id=第九章-上下文工程)
...
```bash
pip install "hello-agents[all]==0.2.8"Copy to clipboardErrorCopied
```
...
## [9.1 什么是上下文工程](https://datawhalechina.github.io/hello-agents/#/./chapter9/第九章 上下文工程?id=_91-什么是上下文工程)
...
![img](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/9-figures/9-1.webp)
...
## [9.2 为什么上下文工程重要](https://datawhalechina.github.io/hello-agents/#/./chapter9/第九章 上下文工程?id=_92-为什么上下文工程重要)
...
### [9.2.1 有效上下文的“解剖学”](https://datawhalechina.github.io/hello-agents/#/./chapter9/第九章 上下文工程?id=_921-有效上下文的解剖学)
...
![img](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/9-figures/9-2.webp)

图 9.2 Calibrating the system prompt

### [9.2.2 上下文检索与智能体式搜索](https://datawhalechina.github.io/hello-agents/#/./chapter9/第九章 上下文工程?id=_922-上下文检索与智能体式搜索)
...
### [9.2.3 面向长时程任务的上下文工程](https://datawhalechina.github.io/hello-agents/#/./chapter9/第九章 上下文工程?id=_923-面向长时程任务的上下文工程)
...
## [9.3 在 Hello-Agents 中的实践：ContextBuilder](https://datawhalechina.github.io/hello-agents/#/./chapter9/第九章 上下文工程?id=_93-在-hello-agents-中的实践：contextbuilder)
...
### [9.3.1 设计动机与目标](https://datawhalechina.github.io/hello-agents/#/./chapter9/第九章 上下文工程?id=_931-设计动机与目标)
...
### [9.3.2 核心数据结构](https://datawhalechina.github.io/hello-agents/#/./chapter9/第九章 上下文工程?id=_932-核心数据结构)
...
class ContextPacket:
    ...
    def __post_init__(self):
        ...
        if self.metadata is None:
            self.metadata = {}
        # 确保相关性分数在有效范围内
        self.relevance_score = max(0.0, min(1.0, self.relevance_score))Copy to clipboardErrorCopied
```
...
class ContextConfig:
    ...
    def __post_init__(self):
        ...
        assert abs(self.recency_weight + self.relevance_weight - 1.0) < 1e-6, \
            "recency_weight + relevance_weight 必须等于 1.0"Copy to clipboardErrorCopied
```

`ContextConfig` 封装了所有可配置的参数，使得系统行为可以灵活调整。特别值得注意的是 `reserve_ratio` 参数，它确保系统指令等关键信息始终有足够的空间，不会被其他信息挤占。

### [9.3.3 GSSC 流水线详解](https://datawhalechina.github.io/hello-agents/#/./chapter9/第九章 上下文工程?id=_933-gssc-流水线详解)
...
def _gather(
    ...
    return packetsCopy to clipboardErrorCopied
```
...
def _calculate_recency(self, timestamp: datetime) -> float:
    ...
    return max(0.1, min(1.0, recency_score))  # 限制在 [0.1, 1.0] 范围内Copy to clipboardErrorCopied
```
...
def _structure(self, selected_packets: List[ContextPacket], user_query: str) -> str:
    ...
    return "\n\n".join(sections)Copy to clipboardErrorCopied
```
...
def _count_tokens(self, text: str) -> int:
    ...
    return int(chinese_chars + english_words * 1.3)Copy to clipboardErrorCopied
```

压缩阶段的设计体现了"保持结构完整性"的原则，即使在 token 预算紧张的情况下，也要尽量保留每个分区的关键信息。

### [9.3.4 完整使用示例](https://datawhalechina.github.io/hello-agents/#/./chapter9/第九章 上下文工程?id=_934-完整使用示例)
...
print("=" * 80)
print(context)
print("=" * 80)Copy to clipboardErrorCopied
```
...
### [9.3.5 最佳实践与优化建议](https://datawhalechina.github.io/hello-agents/#/./chapter9/第九章 上下文工程?id=_935-最佳实践与优化建议)
...
## [9.4 NoteTool：结构化笔记](https://datawhalechina.github.io/hello-agents/#/./chapter9/第九章 上下文工程?id=_94-notetool：结构化笔记)
...
### [9.4.1 设计理念与应用场景](https://datawhalechina.github.io/hello-agents/#/./chapter9/第九章 上下文工程?id=_941-设计理念与应用场景)
...
### [9.4.2 存储格式详解](https://datawhalechina.github.io/hello-agents/#/./chapter9/第九章 上下文工程?id=_942-存储格式详解)
...
1. 重构业务逻辑层
2. 解决依赖冲突问题
3. 提升集成测试覆盖率至85%Copy to clipboardErrorCopied
```
...
### [9.4.3 核心操作详解](https://datawhalechina.github.io/hello-agents/#/./chapter9/第九章 上下文工程?id=_943-核心操作详解)
...
def _build_markdown(self, metadata: Dict, content: str) -> str:
    ...
    # 组合格式
    return f"---\n{yaml_header}---\n\n{content}"Copy to clipboardErrorCopied
```
...
print(f"✅ 笔记创建成功,ID: {note_id}")Copy to clipboardErrorCopied
```
...
def _parse_markdown(self, raw_content: str) -> Tuple[Dict, str]:
    ...
    return metadata, contentCopy to clipboardErrorCopied
```
...
def _update_note(
    ...
    return f"✅ 笔记已更新: {metadata['title']}"Copy to clipboardErrorCopied
```
...
def _search_notes(
    ...
    return results[:limit]Copy to clipboardErrorCopied
```
...
def _delete_note(self, note_id: str) -> str:
    ...
    return f"✅ 笔记已删除: {title}"Copy to clipboardErrorCopied
```

### [9.4.4 与 ContextBuilder 的深度集成](https://datawhalechina.github.io/hello-agents/#/./chapter9/第九章 上下文工程?id=_944-与-contextbuilder-的深度集成)
...
print(summary)Copy to clipboardErrorCopied
```
...
### [9.4.5 最佳实践](https://datawhalechina.github.io/hello-agents/#/./chapter9/第九章 上下文工程?id=_945-最佳实践)
...
## [9.5 TerminalTool：即时文件系统访问](https://datawhalechina.github.io/hello-agents/#/./chapter9/第九章 上下文工程?id=_95-terminaltool：即时文件系统访问)
...
### [9.5.1 设计理念与安全机制](https://datawhalechina.github.io/hello-agents/#/./chapter9/第九章 上下文工程?id=_951-设计理念与安全机制)
...
terminal.run({"command": "head -n 50 src/services/user.py"})  # 按需查看Copy to clipboardErrorCopied
```
...
# 统计错误类型分布
terminal.run({"command": "grep ERROR /var/log/app.log | cut -d':' -f3 | sort | uniq -c"})Copy to clipboardErrorCopied
```
...
# ❌ 不允许的命令: rm
# 允许的命令: cat, cd, cut, dir, du, ...Copy to clipboardErrorCopied
```
...
# 禁止:通过 .. 逃逸
terminal.run({"command": "cd ../../../etc"})  # ❌ 不允许访问工作目录外的路径Copy to clipboardErrorCopied
```
...
# ❌ 命令执行超时（超过 30 秒）Copy to clipboardErrorCopied
```
...
# ... (前10MB的内容) ...
# ⚠️ 输出被截断（超过 10485760 字节）Copy to clipboardErrorCopied
```

通过这四层安全机制，TerminalTool 在提供强大能力的同时，最大程度地保证了系统安全。

### [9.5.2 核心功能详解](https://datawhalechina.github.io/hello-agents/#/./chapter9/第九章 上下文工程?id=_952-核心功能详解)
...
def _execute_command(self, command: str) -> str:
    ...
    try:
        ...
        return f"❌ 命令执行失败: {e}"Copy to clipboardErrorCopied
```
...
def _handle_cd(self, parts: List[str]) -> str:
    ...
    return f"✅ 切换到目录: {self.current_dir}"Copy to clipboardErrorCopied
```
...
# 第四步:查看文件内容
terminal.run({"command": "cat user_service.py"})Copy to clipboardErrorCopied
```

### [9.5.3 典型使用模式](https://datawhalechina.github.io/hello-agents/#/./chapter9/第九章 上下文工程?id=_953-典型使用模式)
...
# 第三步:搜索特定模式
print(terminal.run({"command": "grep -r 'def process' ."}))Copy to clipboardErrorCopied
```
...
# 查找特定时间段的日志
print(terminal.run({"command": "grep '2024-01-19 15:' app.log | tail -n 20"}))Copy to clipboardErrorCopied
```
...
# 查看函数实现
print(terminal.run({"command": "sed -n '/def process_data/,/^def /p' src/processor.py | head -n -1"}))Copy to clipboardErrorCopied
```

### [9.5.4 与其他工具的协同](https://datawhalechina.github.io/hello-agents/#/./chapter9/第九章 上下文工程?id=_954-与其他工具的协同)
...
## [9.6 长程智能体实战：代码库维护助手](https://datawhalechina.github.io/hello-agents/#/./chapter9/第九章 上下文工程?id=_96-长程智能体实战：代码库维护助手)
...
### [9.6.1 场景设定与需求分析](https://datawhalechina.github.io/hello-agents/#/./chapter9/第九章 上下文工程?id=_961-场景设定与需求分析)
...
### [9.6.2 系统架构设计](https://datawhalechina.github.io/hello-agents/#/./chapter9/第九章 上下文工程?id=_962-系统架构设计)

我们的代码库维护助手采用三层架构，如图9.3所示：

![img](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/9-figures/9-3.png)

图 9.3 代码库维护助手三层架构

### [9.6.3 核心实现](https://datawhalechina.github.io/hello-agents/#/./chapter9/第九章 上下文工程?id=_963-核心实现)
...
### [9.6.4 完整使用示例](https://datawhalechina.github.io/hello-agents/#/./chapter9/第九章 上下文工程?id=_964-完整使用示例)
...
### [9.6.5 运行效果分析](https://datawhalechina.github.io/hello-agents/#/./chapter9/第九章 上下文工程?id=_965-运行效果分析)
